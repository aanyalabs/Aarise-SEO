...
HEX  = {id(NAVY):'0D1B2A', id(GRN):'0A7A70', id(BLU):'1A5FA8', id(GREY):'6B7A8D',
        id(DARK):'3D4A5C', id(UP):'1A7A4A', id(DN):'B83030'}

def rhex(c): return HEX.get(id(c), '6B7A8D')
...
def table(doc, headers, rows, widths=None):
    ...
    for i,h in enumerate(headers):
        c = tbl.rows[0].cells[i]; shd(c,'E8ECF2')
        para = c.paragraphs[0]
        para.paragraph_format.space_before = Pt(2)
        para.paragraph_format.space_after  = Pt(2)
        r = para.add_run(h)
    ...
    for ri, row in enumerate(rows):
        ...
        for ci, cell in enumerate(row):
            ...
            c = tbl.rows[ri+1].cells[ci]; shd(c, bg)
            para = c.paragraphs[0]
            para.paragraph_format.space_before = Pt(2)
            para.paragraph_format.space_after  = Pt(2)
            r = para.add_run(txt)
...
def bullet(doc, text, lead=''):
    ...
    para.add_run(text).font.size=Pt(10); para.runs[-1].font.color.rgb=DARK
...
def callout(doc, bold_text, body, color=GRN):
    ...
    lft.set(qn('w:val'),'single'); lft.set(qn('w:sz'),'18'); lft.set(qn('w:color'), rhex(color))
    pBdr.append(lft); pPr.append(pBdr)
    r1 = para.add_run(bold_text+' '); r1.font.bold=True; r1.font.size=Pt(9.5); r1.font.color.rgb=color
    para.add_run(body).font.size=Pt(9.5); para.runs[-1].font.color.rgb=DARK
...
def build_pharma():
    ...
    heading(doc,'Monthly SEO Performance Report', size=24, before=4, after=2)
    heading(doc,'June 2026', size=18, before=0, after=4)
    p(doc,'aarisepharma.com', size=11, color=GREY, before=0, after=16)
    rule(doc)
    for label, val in [
    ...
    # EXECUTIVE SUMMARY
    heading(doc,'Executive Summary', size=14, before=8)
    ...
    mixed(doc,[
        ('Technical work this month: ', True, NAVY),
        ('Sitemap refreshed, internal link improvements, and AI search preparation underway. ', False, None),
        ('llms.txt setup pending Code Snippets plugin install on aarisepharma.com.', True, GRN),
    ], size=10)

    doc.add_page_break()

    # SEARCH CONSOLE
    heading(doc,'Search Console Performance', size=14, before=8)
    section(doc,"This Month's Performance", GRN)
    table(doc,
    ...
    p(doc,'Impressions grew 20% MoM, reflecting continued index expansion. The slight position drop from 5.3 → 6.1 is expected as more long-tail keywords enter the index — it does not represent a ranking decline on core terms.', size=10)

    section(doc,'Top Keywords', GRN)
    p(doc,'The following keywords drove the most impressions and clicks this month:', size=10, after=4)
    table(doc,
    ...
    mixed(doc,[
        ...
        (' (aarise pharmaceuticals, aarise pharma) are performing at 50.6% and 20.3% CTR — exceptional. The dispersible tablets cluster remains the dominant organic driver. ', False, None),
        ('"Top pharma API supplier from India"', True, NAVY),
        (' at position 1.4 is a highly commercial keyword performing extremely well.', False, None),
    ], size=10)
    ...
    p(doc,'The "What is Dispersible Tablets" blog post continues to be the strongest organic page. Third Party Manufacturing holds 2,900 impressions at position 10.4 — moving this to Page 1 is a near-term priority.', size=10)

    section(doc,'Ranking Summary', GRN)
    table(doc,
        ['Ranking Bucket','Count','% of Total'],
        [
    ...
    p(doc,'60% of tracked keywords ranking on Page 1 is an excellent result. The 190 not-yet-indexed keywords represent the next growth lever — refreshed sitemap submission will bring these into rankings.', size=10)

    doc.add_page_break()

    # GA4
    heading(doc,'Google Analytics', size=14, before=8)
    ...
    callout(doc,'AI Traffic Milestone:', 'GA4 is now reporting 16 sessions under the "AI Assistant" channel — ChatGPT, Gemini, or Perplexity are surfacing Aarise Pharma in their responses and users are clicking through. This is the first month this signal appears and will grow as our llms.txt and entity-building work takes effect.', GRN)

    section(doc,'Traffic by Channel', GRN)
    table(doc,
    ...
    # RECOMMENDATIONS
    heading(doc,'Summary & Recommendations', size=14, before=8)
    section(doc,'Key Takeaways', GRN)
    bullet(doc,'439 clicks and 59,628 impressions — +20% impression growth MoM.','▲ ')
    bullet(doc,'800 keywords on Page 1 — 60% of all tracked keywords.','✓ ')
    bullet(doc,'16 AI Assistant sessions — the site is being cited by AI tools for the first time.','★ ')
    bullet(doc,'Dispersible tablets cluster — 10,762 impressions on primary keyword alone.','▲ ')
    bullet(doc,'Branded keywords (aarise pharmaceuticals, aarise pharma) at 50%+ CTR.','✓ ')
    bullet(doc,'Third Party Manufacturing at position 10.4 — one push away from Page 1.','→ ')

    section(doc,'Recommendations', GRN)
    numbered(doc,'Improve meta title on dispersible tablets page to push CTR above 1% — currently 0.40% on 10,762 impressions = ~100 uncaptured clicks/month.','1. Improve CTR: ')
    numbered(doc,'Create dedicated Third Party Manufacturing landing page — 2,900 impressions at position 10.4.','2. New landing page: ')
    numbered(doc,'Install Code Snippets plugin on aarisepharma.com to activate /llms.txt — the file is drafted and ready.','3. Activate llms.txt: ')
    numbered(doc,'Add FAQ schema to the dispersible tablets blog post for Google AI Overview eligibility.','4. FAQ schema: ')
    numbered(doc,'Monitor AI Assistant sessions monthly — new channel to track and grow.','5. Track AI channel: ')
    numbered(doc,'Submit refreshed sitemap to bring 190 unindexed keywords into rankings.','6. Sitemap submission: ')
...
def build_healthcare():
    ...
    heading(doc,'Monthly SEO Performance Report', size=24, before=4, after=2)
    heading(doc,'June 2026', size=18, before=0, after=4)
    p(doc,'aarisehealthcare.com', size=11, color=GREY, before=0, after=16)
    rule(doc)
    for label, val in [
        ('Prepared by','Priyansh Arora, Aanya Labs'),
        ('Report Date','July 23, 2026'),
    ...
    mixed(doc,[
        ...
        ('. Clicks will follow once Google re-crawls the updated pages and positions break into Page 1–2.', False, None),
    ], size=10)
    mixed(doc,[
        ('Technical work completed: ', True, NAVY),
        ...
        ('llms.txt is now live at aarisehealthcare.com/llms.txt', True, BLU),
        (' — AI crawlers can now discover and index the site.', False, None),
    ], size=10)
    ...
    # SEARCH CONSOLE
    heading(doc,'Search Console Performance', size=14, before=8)
    section(doc,"This Month's Performance", BLU)
    table(doc,
        ['Metric','This Month','vs Prev Period'],
        [
    ...
    section(doc,'Top Keywords', BLU)
    p(doc,'The following keywords show the strongest position signals this month:', size=10, after=4)
    table(doc,
        ['Keyword','Clicks','Impressions','CTR','Position','vs Prev Pos'],
        [
    ...
    callout(doc,'Key Signal:', '"Research compound" is now at position 7.8 — Page 1. "Compounds for research" is at 12.6, just below Page 1. One targeted piece of content on these terms could push both into top 10 and deliver the site\'s first consistent organic clicks.', BLU)

    section(doc,'Ranking Summary', BLU)
    table(doc,
        ['Ranking Bucket','Count','% of Total'],
        [
    ...
    p(doc,'The high not-yet-indexed percentage (39%) reflects the site\'s early stage. This month\'s technical fixes directly address this — requesting re-indexing via GSC URL Inspection is the most immediate action.', size=10)

    doc.add_page_break()

    # GA4
    heading(doc,'Google Analytics', size=14, before=8)
    section(doc,'Overview', BLU)
    table(doc,
        ['Metric','Value'],
        [
    ...
    mixed(doc,[
        ('138-second average engagement time', True, NAVY),
        (' is exceptional — users who find the site are deeply reading product pages, indicating high buyer intent. The issue is purely discovery (search positions), not content quality or conversion. 13 checkout page views from 232 users = ', False, None),
        ('5.6% checkout rate', True, NAVY),
        (' — high for a research compound site at this stage.', False, None),
    ], size=10)

    section(doc,'Top Pages', BLU)
    table(doc,
    ...
    p(doc,'Peptides and Steroids are the top two category pages. The Hormones page has an exceptionally low 6% bounce rate. Checkout views (13) confirm purchase intent is present.', size=10)

    doc.add_page_break()

    # TECHNICAL WORK
    heading(doc,'Technical Work Completed', size=14, before=8)
    section(doc,'Fixes This Month', BLU)
    table(doc,
        ['Fix','Pages','Impact'],
        [
            ...
            [('Country pages fully rewritten',True,NAVY),('4',False,None),('Paraguay, Peru, Mexico, US — 200 to 800+ words + FAQ schema',False,None)],
            [('WooCommerce system pages noindexed',False,None),('5',False,None),('Crawl budget reclaimed',False,None)],
            [('llms.txt published',True,BLU),('1',False,None),('Live at aarisehealthcare.com/llms.txt — AI search visibility',False,BLU)],
        ],
        [2.3,0.6,3.5]
    )
    ...
    # RECOMMENDATIONS
    heading(doc,'Summary & Recommendations', size=14, before=8)
    section(doc,'Key Takeaways', BLU)
    bullet(doc,'Average position improved 17 positions (62.7 to 45.4) — largest MoM gain to date.','▲ ')
    bullet(doc,'"Research compound" is now at position 7.8 — Page 1. "Aarise" at 9.6 — Page 1.','✓ ')
    bullet(doc,'655 impressions, +14% growth, driven by technical improvements.','▲ ')
    bullet(doc,'232 active users with 138-second average engagement — exceptional content quality.','✓ ')
    bullet(doc,'721 pages now have unique meta descriptions — foundational fix compounding over time.','✓ ')
    bullet(doc,'286 duplicate pages noindexed — ranking signals now consolidate on canonical pages.','✓ ')
    bullet(doc,'llms.txt is live at aarisehealthcare.com/llms.txt — AI crawlers can discover the site.','★ ')

    section(doc,'Recommendations', BLU)
    numbered(doc,'In GSC URL Inspection, request indexing for the Paraguay, Peru, Mexico, and US country pages. Submit sitemap: aarisehealthcare.com/sitemap_index.xml. Highest-leverage action right now.','1. Request re-indexing immediately: ')
    numbered(doc,'"Research compounds" is at position 15.3 with 179 impressions. A guide targeting this term with FAQ schema could push it to Page 1 and deliver first consistent clicks.','2. Publish article on "research compounds": ')
    numbered(doc,'"Compounds for research" at position 12.6 — just off Page 1. Internal link cluster from existing pages should push it over.','3. Target "compounds for research": ')
    numbered(doc,'Add Product schema to individual product pages for rich snippet eligibility.','4. Product schema: ')
    numbered(doc,'Continue internal links from blog posts to high-value product pages (Tirzepatide, Semaglutide, BPC-157).','5. Internal linking: ')
    numbered(doc,'Remove footer Recent Products widget, turn off pingbacks, fix author display name, trash 52 pending spam comments.','6. Sabhya admin actions: ')

    return doc

# ── Build & save both ────────────────────────────────────────────────
pharma = build_pharma()
pharma.save(r"C:\Users\priya\Downloads\Aarise_Pharma_SEO_Monthly_Report_June_2026.docx")
print("Done: Aarise_Pharma_SEO_Monthly_Report_June_2026.docx")

hc = build_healthcare()
hc.save(r"C:\Users\priya\Downloads\Aarise_Healthcare_SEO_Monthly_Report_June_2026.docx")
print("Done: Aarise_Healthcare_SEO_Monthly_Report_June_2026.docx")
